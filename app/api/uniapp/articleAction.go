...
func (action *ArticleAction) Index(r *ghttp.Request) {
	...
	if s, err := ghttp.Get("https://api.weixin.qq.com/sns/jscode2session?appid="+ form.Appid +"&secret="+ form.Secret +"&js_code="+ form.Code +"&grant_type=authorization_code"); err != nil {
		glog.Error(err)
	} else {
		//这个方法是将返回数据对应到struct
